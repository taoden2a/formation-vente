...
✅ RÈGLE CRITIQUE Vercel + Supabase (2026-03-21) — **Problème** : `DATABASE_URL` avec port 5432 (connexion directe) → Vercel serverless ne peut pas atteindre Supabase sur ce port → `Can't reach database server at db.xxx.supabase.co:5432` — **Règle** : sur Vercel, `DATABASE_URL` DOIT utiliser le **Transaction Pooler** (port 6543) depuis Supabase → Settings → Database → Connection string → mode "Transaction" → URL format `postgresql://postgres.PROJECT_ID:PASSWORD@aws-0-REGION.pooler.supabase.com:6543/postgres` — `DIRECT_URL` garde le port 5432 (utilisé seulement pour les migrations locales, pas en runtime Vercel) — **Scénario typique du bug** : token créé en local (port 5432 ✅), email link pointe vers prod (NEXTAUTH_URL .env.local), vérification sur Vercel échoue (port 5432 ❌ depuis Vercel) → token valide en DB mais inaccessible depuis Vercel
